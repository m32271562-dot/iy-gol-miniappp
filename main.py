...
def get_matches():
  ...
  # ----------------------------------------------------
  # 1. CANLI MAÇLAR (En az 3 Toplam Şut VE 2 İsabetli Şut)
  # ----------------------------------------------------
  try:
    ...
    if res_live.status_code == 200:
      ...
      for event in events:
        ...
        try:
          ...
          res_stats = requests.get(stats_url, headers=headers, timeout=4)
                ...
                for group in period.get("groups", []):
                  for item in group.get("statisticsItems", []):
                    ...
                    if "shots on target" in name_lower:
                      home_on_target = int(item.get("home", 0))
                      away_on_target = int(item.get("away", 0))
                    elif "total shots" in name_lower or "shots" == name_lower:
                      home_shots = int(item.get("home", 0))
                      away_shots = int(item.get("away", 0))
        except:
          pass
        ...
        if home_shots >= 3 and home_on_target >= 2:
          prediction_text = f"{home_team} GOL ATAR"
          shots_info = f"Şut: {home_shots} (İsabet: {home_on_target})"
        elif away_shots >= 3 and away_on_target >= 2:
          prediction_text = f"{away_team} GOL ATAR"
          shots_info = f"Şut: {away_shots} (İsabet: {away_on_target})"
  ...
  try:
    ...
    if res_sched.status_code == 200:
      ...
      for event in sched_events:
        ...
        # Önümüzdeki 3 saat içinde başlayacak maçlar
        if 0 < time_diff <= 180:
