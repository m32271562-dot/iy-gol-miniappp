...
def parse_signal_data(sig):
    """API'den gelen karmaşık JSON verisini detaylıca ayıklar."""
...
async def fetch_signals():
    """Gece yarısı tarih kaymasını önleyen ve çok sayfalı veri çeken motor."""
...
async def track_signals_loop():
    """İlk açılışta eski maçları sessizce hafızaya alan ve sadece YENİ sinyalleri bildiren döngü."""
    ...
    while True:
        try:
            ...
            if is_first_run:
                # İlk çalıştırmada mevcut tüm sinyalleri hafızaya al ama Telegram'a mesaj ATMA!
                for sig in signals:
                ...
                is_first_run = False
                logging.info(f"Sistem Isındı: {len(TRACKED_SIGNALS)} eski sinyal hafızaya alındı, bildirimler aktifleştirildi.")
            else:
                # Canlı Takip Modu
                for sig in signals:
                    sig_id = sig["id"]
                    ...
                    # 2. VAR OLAN SİNYAL SONUÇLANDIĞINDA
                    else:
